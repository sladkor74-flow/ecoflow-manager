...
export default async function(req) {
  ...
  try {
    ...
    const prompt = [
      'Sei l\'Assistente del gestionale di SMOCO Srl, azienda che raccoglie e gestisce pneumatici fuori uso (PFU, codice EER 16 01 03, rifiuti speciali non pericolosi) per il sistema collettivo Ecotyre nel Sud Italia.',
      'Aiuti la direzione con i dubbi sulle norme (D.Lgs. 152/2006, DM 182/2019 sui PFU, RENTRI e DM 59/2023, Albo nazionale gestori ambientali e responsabile tecnico), con le domande sui dati della commessa e con la preparazione all\'esame di responsabile tecnico gestione rifiuti.',
      '',
      `Data di oggi: ${oggi}. Utente: ${user.full_name || user.email || ''}.`,
      '',
      'REGOLE',
      `1. Parti dalla base di conoscenza qui sotto, verificata alla data indicata voce per voce. Per le domande sulle norme controlla online sulle fonti ufficiali (${FONTI_UFFICIALI.join('; ')}) se ci sono novita' successive: se ne trovi, applica la norma piu' recente, dillo chiaramente nella risposta e riportala in novita_normative indicando la voce da aggiornare (voce_id) e il testo aggiornato proposto.`,
      ...
      '3. Per le domande sui dati usa solo i DATI DEL GESTIONALE forniti. Se un dato manca dillo e indica in quale sezione del gestionale trovarlo (Target & Status, Verifiche, Giacenze, Qualifica Fornitori, Fatturazione). Non inventare numeri.',
    ...
    // Novita' normative: proposte da approvare nella base di conoscenza.
    const novita = (Array.isArray(esito.novita_normative) ? esito.novita_normative : []).filter(n => n && n.descrizione).slice(0, 3);
    let proposte = 0;
    if (novita.length) {
      const inAttesa = await svc.ConoscenzaAssistente.filter({ stato: 'proposta' }, '-created_date', 200).catch(() => []);
      for (const n of novita) {
        const voce = BASE_CONOSCENZA.find(v => v.id === n.voce_id);
        if (voce && inAttesa.some(p => p.voce_id === voce.id)) continue;
        await svc.ConoscenzaAssistente.create({
          tipo: 'aggiornamento_normativo',
          stato: 'proposta',
          attiva: true,
          area: voce ? voce.area : 'tua',
          titolo: taglia(n.titolo || (voce ? voce.titolo : 'Novita\' normativa'), 200),
          testo: String(n.testo_proposto || n.descrizione),
          voce_id: voce ? voce.id : '',
          testo_precedente: voce ? voce.testo : '',
          motivazione: String(n.descrizione),
