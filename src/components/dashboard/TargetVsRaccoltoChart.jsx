  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey="regione" tick={{ fontSize: 11 }} />
        <YAxis tick={{ fontSize: 11 }} />
        <Tooltip formatter={(v) => v.toLocaleString('it-IT', { maximumFractionDigits: 1 }) + ' t'} />
        <Legend />
        <Bar dataKey="target" name="Target" fill="hsl(197 82% 22%)" radius={[4, 4, 0, 0]} />
        <Bar dataKey="raccolto" name="Raccolto" fill="hsl(198 93% 60%)" radius={[4, 4, 0, 0]} />
      </BarChart>
    </ResponsiveContainer>
  );
}