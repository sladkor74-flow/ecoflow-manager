...
export default function TargetStatus() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [raccoltoRes, targetRes, impTargetRes] = await Promise.all([
        base44.functions.invoke('computeRaccolto', {}),
  ...
  const impiantiData = useMemo(() => {
    ...
    return raccolto.by_impianto.map((i) => ({
      impianto: i.impianto,
      totale: i.totale,
      mesi: MESI.map((m) => ({
        mese: m,
  ...
  }, [raccolto, impiantoTargets]);

  // KPIs
  const kpis = useMemo(() => {
    const targetAnnuoTotale = mergedData.reduce((s, r) => s + r.targetAnnuo, 0);
